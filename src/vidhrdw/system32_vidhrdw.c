	...
	*/
	...
	if (rowscroll || rowselect) {
		...
		for (line = 0; line < 224;line++) {
			...
			tilemap_set_scrolly(system32_layer_tilemap[layer],0, (yscroll & 0x1ff));								tilemap_set_scrolldx(system32_layer_tilemap[layer], (sys32_videoram[(0x01FF30+layer*4)/2]&0x1ff)+monitor*monitor_res, -(sys32_videoram[(0x01FF30+layer*4)/2]&0x1ff)-monitor*monitor_res);
			tilemap_set_scrolldy(system32_layer_tilemap[layer], sys32_videoram[(0x01FF32+layer*4)/2]&0x1ff, -sys32_videoram[(0x01FF32+layer*4)/2]&0x1ff);	tilemap_draw(bitmap,&clip,system32_layer_tilemap[layer],trans,0);
		}
	}
	else {
		// Multi32: Shift layer 3's rowscroll left one screen so that it lines up
